...
def train(model, train_dataset, train_loader, lr_scheduler, criterion, optimizer, epoch):
    model.train()
    lr_scheduler.step()
    train_acc_list = []
    train_loss_list = []
    for i, (input, targets, target_weights, data_idx) in enumerate(train_loader):
...
def val(model, val_dataset, val_loader, criterion, epoch):
    model.eval()
    val_acc_list = []
    val_loss_list = []
    with torch.no_grad():
        for i, (input, targets, target_weights, data_idx) in enumerate(val_loader):
            targets = targets.cuda(non_blocking=True)
            target_weights = target_weights.cuda(non_blocking=True)
...
for epoch in range(0, 5000):
    ...
    train_loss, train_acc = train(model, train_dataset, train_loader, lr_scheduler, criterion, optimizer, epoch)
    ...
    val_loss, val_acc = val(model, val_dataset, val_loader, criterion, epoch)
