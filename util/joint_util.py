...
def get_avg_preds(batch_heatmaps, threshold = 0.0):
    preds = []
    maxvals = []
...
def accuracy(output, target, hm_pred = "get_avg_preds", hm_type='gaussian', thr=0.5):
    '''
    ...
    norm = 1.0
    if hm_type == 'gaussian':
        pred, _ = eval(hm_pred)(output)
        target, _ = eval(hm_pred)(target)
        h = output.shape[2]
        w = output.shape[3]
