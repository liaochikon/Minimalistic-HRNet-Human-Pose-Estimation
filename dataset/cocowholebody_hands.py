...
class COCOWholebody_LeftHand(Dataset):
    def __init__(self, anno_path, image_root_path,
        ...
        for raw_image_id in self.raw_image_ids:
            ...
            for person_anno in people_anno:
                if person_anno['lefthand_valid'] == 0:
                    continue

                bbox = person_anno['lefthand_box']
                ...
                lefthand_keypoints = person_anno['lefthand_kpts']
                keypoints = np.array([*lefthand_keypoints], dtype=np.float)
